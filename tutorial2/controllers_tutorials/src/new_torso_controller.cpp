...
class MyTorsoController : public controller_interface::ControllerBase
{
public:
    ...
    void update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        if( (time.nsec % 5000000) == 0)   // Coment this line when working on the real robot
        {
            ...
            cont+=0.01;
            double pos = 0.0;
            joints_[0].setCommand(0.15*sin(cont) + 0.15);
            //ROS_WARN_STREAM("sin " << joints_[1].getPosition());
            ROS_INFO_STREAM("position:"<<joints_[0].getPosition()<<"calculated position:"<<0.15*sin(cont) + 0.15);
            //IMU
...
PLUGINLIB_DECLARE_CLASS(controllers_tutorials, MyTorsoController, controllers_tutorials::MyTorsoController, controller_interface::ControllerBase);
