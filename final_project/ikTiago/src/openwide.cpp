...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "openwide");
  ros::NodeHandle nh; 
  ros::ServiceServer get_transform;
  get_transform=nh.advertiseService("/openwide", reachCallback);
  ros::spin();
  return 0;
}
...
void test(ros::NodeHandle& nh, std::string urdf_param, double theta, double x, double y, double z)
{
  ...
  **********************************************************************/
  ...
  //theta=theta+0.01;
    ...
    if (rc>=0)
    {
      ...
      //   }
      // }
      goal.trajectory.points[index].time_from_start = ros::Duration(index*0.5+5);
  ...
  *****************************************************************************************/
  ...
  for(int l=0;l<times;l++)
  {
    ...
    waypoint.trajectory.points[l].positions[0] = Jointstate0(0)+l*0.012;
    waypoint.trajectory.points[l].positions[1] = Jointstate0(1);
    waypoint.trajectory.points[l].positions[2] = Jointstate0(2);
    waypoint.trajectory.points[l].positions[3] = Jointstate0(3)+l*0.016;
    waypoint.trajectory.points[l].positions[4] = Jointstate0(4);
    waypoint.trajectory.points[l].positions[5] = 1.39-l*0.02;
    ...
    //  duration
    waypoint.trajectory.points[l].time_from_start = ros::Duration(l+1.0);
